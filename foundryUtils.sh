...
then
	...
	then
		...
		then
			...
			eval "docker run -v $build-volume:/opt -v /var/run/docker.sock:/var/run/docker.sock $exposePorts $DOCKER_NETWORK_OPT --name $build --net=host $volumeList $build"
		...
		then
			...
			docker run -v $build-volume:/opt -v /var/run/docker.sock:/var/run/docker.sock --net=host  --entrypoint bash -i -t --rm $build
			exit 0
